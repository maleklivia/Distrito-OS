const Cardapio = {
  ...
  payload(form){const data=new FormData(form),fulfillmentType=data.get('fulfillment');return{customer:{name:data.get('name'),phone:data.get('phone')},fulfillmentType,address:{postalCode:data.get('postalCode')||'',city:data.get('city')||'',street:data.get('street')||'',number:data.get('number')||'',district:data.get('district')||'',complement:data.get('complement')||'',reference:data.get('reference')||''},paymentMethod:data.get('payment'),notes:data.get('notes')||'',adultConfirmed:data.get('adultConfirmed')==='on',website:data.get('website')||'',items:this.cartData().map(i=>({productId:i.product.id,quantity:i.quantity}))}},
  ...
  message(payload){const lines=['*NOVO PEDIDO — PETISBAR TEODORO*','',...this.cartData().map(i=>`• ${i.quantity}x ${i.product.name} — ${this.money(Number(i.product.sale_price)*i.quantity)}`),'',`*Subtotal:* ${this.money(this.subtotal())}`,`*Recebimento:* ${payload.fulfillmentType==='entrega'?'Entrega':'Retirada'}`,`*Pagamento:* ${payload.paymentMethod}`,`*Cliente:* ${payload.customer.name}`,`*Telefone:* ${payload.customer.phone}`];if(payload.fulfillmentType==='entrega'){lines.push('', '*ENDEREÇO DE ENTREGA*',`${payload.address.street}, ${payload.address.number}`,`${payload.address.district} — ${payload.address.city}`,`CEP: ${payload.address.postalCode}`);if(payload.address.complement)lines.push(`Complemento: ${payload.address.complement}`);if(payload.address.reference)lines.push(`Referência: ${payload.address.reference}`)}if(payload.notes)lines.push(`*Observações:* ${payload.notes}`);lines.push('','Aguardando confirmação do estabelecimento.');return lines.join('\n')},
